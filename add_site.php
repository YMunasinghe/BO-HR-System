...
<body>
	<div class="main-wrapper">
		...
		<div class="page-wrapper">
            ...
            <li class="breadcrumb-item active" aria-current="page"><i data-feather="chevron-right"></i>Add Department </a></li>
    ...
    <div class="mb-3">
        ...
        <select class="js-example-basic-single form-select" data-width="100%" name="company" id="company">
            <?php
            $result = mysqli_query($conn, "SELECT * FROM company ORDER BY C_id");
            while ($row = mysqli_fetch_array($result)) {
                ?>
                <option value="<?php echo $row["C_id"]; ?>"><?php echo $row["C_name"]; ?> (<?php echo $row["C_EPFNo"]; ?>)</option>
            <?php } ?>
        </select>
    </div>
    ...
    <div class="mb-3">
        ...
        <select class="js-example-basic-single form-select" data-width="100%" name="department" id="department">
            <option value="">Select a department</option>
										...
										<div  id="dynamic-rows">
											...
											<h4 class="card-title">Department 01</h4>

											<div class="col-sm-10">
												<div class="mb-3">
													<label class="form-label">Department Name</label>
													<input type="text" class="form-control" placeholder="Department" name="COM_Department[]">
												</div>
											</div><!-- Col -->
									...
									<button id="submit-form-btn" type="submit" class="btn btn-success" name="add_comd">Add Employment</button>

									</form>
									<?php include_once 'PHP/WRITE/C_department_management-Script.php'; ?>
							</div>
						</div>
					</div>
				</div>
...
document.getElementById('company').addEventListener('change', function() {
    ...
    // If a company is selected, fetch departments using AJAX
    if (companyId !== '') {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                // Create option elements for each department
        ...
        xhr.open('GET', 'PHP/READ/get_departments.php?company=' + companyId, true);
        xhr.send();
    }
});
</script>
  ...
  $('#add-row-btn').click(function() {
    ...
    clonedRow.find('.card-title').text('Department ' + lastMemberNumber);
